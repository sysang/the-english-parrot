...
class DialogueTurn(object):
    def __init__(self, lesson, scenario, progress, intention, utterances, question):
    ...
    def get_action_initialing_slots(self):
        return self.json_stringify_slot_values({'progress': 0, 'lesson_topic': self.lesson})

    def json_stringify_slot_values(self, custom={}):
        lesson_progress = '{lesson}_progress'.format(**{'lesson': self.lesson})
        data = {
                lesson_progress: self.lesson,
                'progress': self.progress if 'progress' not in custom else custom['progress'],
    ...
    def parse_entities(self, random=False):
        if random:
...
def dialogue_initialize_lesson(turn):
    template = Template(
            "* client_initialize_{{ turn.lesson }}_story\n"
